...
def main():
    ...
    ap.add_argument("--heal-iterations", type=int, default=4,
                    help="A lokális nyakjavítás maximális ismétlésszáma")
    ap.add_argument("--draft", action="store_true", help="Hibás geometria csak jelölt piszkozatként")
    a = ap.parse_args()
    ...
    with output_directory(final_out) as out:
        ...
        for cont in sorted(by_cont):
            ...
            for i, p in enumerate(parts):
                ...
                if a.heal:
                    n0 = necks(p, a.min_web)
                    if n0:
                        p = heal_to_convergence(p, min_web=a.min_web, max_iter=a.heal_iterations)
                        healed.append((cont, n0, necks(p, a.min_web)))
                ...
                for n, g in cgeoms:
                    ...
                    if not a.no_labels:
                        ...
                        if n not in label_candidates or big.area > label_candidates[n][0]:
                            label_candidates[n] = (big.area, (n.upper(), c.x, c.y, hgt))
                if inner:
        ...
        labels = [value[1] for value in label_candidates.values()]

        # ---- csempezes a lezerágy-korláthoz ---------------------------------
        tiled, oversize = [], 0
        for cont, i, p in pieces:
            zones = tile_piece(p, max_w, max_h, min_area=a.min_island, min_web=a.min_web)
            if len(zones) > 1:
                oversize += 1
            for q, (r, c) in zones:
        ...
        for _c, _i, sgeom in scores:
            # Blender's SVG importer ignores stroke width on open paths.
            # Expand only the RENDER overlay to filled ribbons; cut/score files
            # retain their original centerlines and operation colors.
            for part in polys(sgeom.simplify(0.06).buffer(0.10, quad_segs=1).simplify(0.015)):
                d = [ring_d(part.exterior.coords, H)]
                d.extend(ring_d(r.coords, H) for r in part.interiors)
                eb.append(f'  <path d="{" ".join(d)}" fill="#482e20" fill-rule="evenodd"/>')
        for n, x, y, hgt in labels:
        ...
        # ---- riport ---------------------------------------------------------
        if not tiled:
            raise ValueError("No cuttable map pieces remain; adjust size or island threshold")
        biggest = max(tiled, key=lambda t: max(t[2].bounds[2] - t[2].bounds[0],
                                               t[2].bounds[3] - t[2].bounds[1]))
        bb = biggest[2].bounds
        neck_total = sum(necks(q, a.min_web) for _c, _i, q, _rc, _t in tiled)
        sizes_ok = all(q.bounds[2] - q.bounds[0] <= max_w + 1e-6 and
                       q.bounds[3] - q.bounds[1] <= max_h + 1e-6
                       for _c, _i, q, _rc, _t in tiled)
        # An entirely thin island erodes to nothing and has zero necks.
        # Measure its inscribed width as well; neck count alone is insufficient.
        weakest = min((widest_inscribed(q, hi=max(12.0, a.min_web * 2))
                       for _c, _i, q, _rc, _t in tiled), default=0.0)
        report = {
            "all_ok": bool(tiled) and neck_total == 0 and sizes_ok and weakest >= a.min_web,
            "weakest_mm": round(weakest, 3),
            "min_web_mm": a.min_web,
