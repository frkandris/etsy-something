...
# ALWAYS pass a style reference. Text alone produced a different shape
# language every time; the reference is what pins the look to the market.
REF=product/pipeline/references/motif-abstract-dog.png
gen () { .venv/bin/python product/pipeline/00_generate.py --subject-text "$2 $COMMON" --name "$1" \
         --levels 7 --ref $REF --out $OUT 2>&1 | grep KB; }
